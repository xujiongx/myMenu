...
**当前能力摘要**：账号密码登录；**每人独立菜单**（分类/菜品/订单按 `user_id` 隔离）；点菜结算、订单记录、自助菜品管理与 ImgBB 上传；lucide-react 图标；菜单缓存标签 `menu:{userId}`；品牌名 **小菜单**；PWA（manifest + Serwist Service Worker，安装图标 `/icons/icon-*.png`）。
